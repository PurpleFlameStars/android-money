...
public class BunRefreshLayout extends ViewGroup {
    private static final int LOADING_DISTANCE = 34;  //loading 位置
    ...
    public BunRefreshLayout(Context context) {
        this(context, null);
    }

    public BunRefreshLayout(Context context, AttributeSet attrs) {
        super(context, attrs);

        mDecelerateInterpolator = new DecelerateInterpolator(DECELERATE_INTERPOLATION_FACTOR);
        mTouchSlop = ViewConfiguration.get(context).getScaledTouchSlop();
        defaultDuration = getResources().getInteger(android.R.integer.config_mediumAnimTime);
        ...
        int pullcolor=getResources().getColor(R.color.bun_pull_color) ;
        mColorSchemeColors = new int[]{pullcolor};
    ...
    public void setTipColor(int bgColor, int textColor) {
        mTipDrawable.setColor(bgColor);
        mTipDrawable.setTextColor(textColor);
    }
...
//        mRefreshView.measure(MeasureSpec.makeMeasureSpec(mRefreshViewWidth, MeasureSpec.EXACTLY), MeasureSpec.makeMeasureSpec(mRefreshViewHeight, MeasureSpec.EXACTLY));
    ...
    public void setToCorrectPosition(int durationToCorrectPosition) {
        mDurationToCorrectPosition = durationToCorrectPosition;
    }

    public void resetCorrectPositionTime() {
        mDurationToCorrectPosition = defaultDuration;
    }
    ...
    private void animateOffsetToCorrectPosition() {
        ...
        mAnimateToCorrectPosition.setDuration(100);
    ...
    private final Animation mAnimateToTipPosition = new Animation() {
        @Override
        public void applyTransformation(float interpolatedTime, Transformation t) {
            //  int endTarget = mLoadingFinalOffset; 使刷新文本高度与图片高度保持一致
            int endTarget = mSpinnerFinalOffset;
            int targetTop = (mFrom + (int) ((endTarget - mFrom) * interpolatedTime));
            int offset = targetTop - mTarget.getTop();
            setTargetOffsetTop(offset, false /* requires update */);
